...
async def main_menu_markup() -> InlineKeyboardMarkup:
    ...
    to_purchases_button = InlineKeyboardButton(text='К покупкам 🛍️', callback_data='to_purchases_handler')
    exit_button = InlineKeyboardButton(text='Выйти 🚪', callback_data='exit')

    return InlineKeyboardMarkup(inline_keyboard=[
        [show_account_info_button, to_purchases_button],
        [exit_button],
    ])
...
async def choose_city_markup() -> InlineKeyboardMarkup:
    ...
    builder.adjust(1, 1, repeat=True)
...
async def confirm_choice_markup() -> InlineKeyboardMarkup:
    ...
    confirm_choice_button = InlineKeyboardButton(text='Подтвердить ✅', callback_data='confirm_choice')
    refuse_choice_button = InlineKeyboardButton(text='Назад 👈', callback_data='refuse_choice')

    keyboard = InlineKeyboardMarkup(
        inline_keyboard=[
            [confirm_choice_button],
            [refuse_choice_button],
        ]
    )
